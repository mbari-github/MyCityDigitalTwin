...
class SimulationSynchronization(object):
    ...
    def tick(self):
        ...
        # -------------------------------------
        # Aggiornamento del dizionario con le informazioni di tutti i veicoli
        # -------------------------------------
        try:
            vehicle_ids = traci.vehicle.getIDList()

            for vehicle_id in vehicle_ids:
                # Otteniamo l'ID del veicolo in CARLA utilizzando il mapping sumo2carla_ids
                ...
                if carla_actor_id is not None:
                    ...
                    CO2emission = traci.vehicle.getCO2Emission(vehicle_id)
                    COemission = traci.vehicle.getCOEmission(vehicle_id)
                    PMxEmission = traci.vehicle.getPMxEmission(vehicle_id)
...
if __name__ == '__main__':
    ...
    logging.debug(f"SUMO port: {arguments.sumo_port}")
    logging.debug(f"SUMO GUI: {arguments.sumo_gui}")
    logging.debug(f"TLS Manager: {arguments.tls_manager}")  
    logging.debug(f"Sync Vehicle Lights: {arguments.sync_vehicle_lights}")
    logging.debug(f"Sync Vehicle Color: {arguments.sync_vehicle_color}")
    logging.debug(f"SUMO Config File: {arguments.sumo_cfg_file}")
